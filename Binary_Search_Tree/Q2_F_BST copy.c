...
void inOrderTraversal(BSTNode *root)
{
	...
	while(1){
		...
		if(stk->top->data->left != NULL && stk->top->data->left->item != NULL){			// top의 left값이 null이 아니면
			push(stk,stk->top->data->left);			// top의 left 스택에 집어넣기
			continue;
		}
		else if (stk->top->data->left == NULL || stk->top->data->left->item == NULL){	// top의 left값이 null이면, 스택에서 top 빼고 출력하기
			BSTNode* poped= pop(stk);			
			printf("\n%d\n",poped->item);

			if(poped->right != NULL && poped->right->item != NULL){
				push(stk,poped->right);
			}
			poped->item = NULL;						// 실제 top노드값도 null로 만들기
		}
	}
