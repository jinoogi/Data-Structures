...
void RecursiveReverse(ListNode **ptrHead)
{	ListNode *post=(*ptrHead)->next;
	if(((*ptrHead)->next->next) != NULL){
		RecursiveReverse(&((*ptrHead)->next));	
	}
	else if(((*ptrHead)->next->next) == NULL){
	...
	post->next=*ptrHead;   	//역방향 연결
	ListNode* temp = (*ptrHead)->next;
	(*ptrHead)->next=NULL; 	//연결끊기
	*ptrHead = temp;		//연탄나르기식 끝노드 전달받음
	// printf("%d",*ptrHead);
