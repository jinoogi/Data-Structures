...
{	int i=0; //스택에 남은거 있는지 여부로 밸런스 판단하자는 로직
	...
	for(i;i<256;i++){
		...
		if(i>0){
			if(expression[i]==']'){
				int item=pop(s);
				if(item==3){ 		//스택에서 뽑은게 [ 이면
									//원상복구 안시킴
				}
				else if(item!=3){ 	//스택에서 뽑은게 [이 아니면
				push(s,item); 		//다시 집어넣고
				push(s,-3); 		// ] 도 집어넣기
				}
			}
				...
				int item=pop(s);
				if(item==2){ 		//스택에서 뽑은게 { 이면
									//원상복구 안시킴
				}
				else if(item!=2){ 	//스택에서 뽑은게 {이 아니면
				push(s,item); 		//다시 집어넣고
				push(s,-2); 		// } 도 집어넣기
				}
			}
				...
				int item=pop(s);
				if(item==1){ 		//스택에서 뽑은게 ( 이면
									//원상복구 안시킴
				}
				else if(item!=1){ 	//스택에서 뽑은게 (이 아니면
				push(s,item); 		//다시 집어넣고
				push(s,-1); 		// ) 도 집어넣기
				}
			}
			else{
				if(expression[i]=='[')push(s,3);
				if(expression[i]=='{')push(s,2);
